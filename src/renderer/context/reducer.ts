...
export type AppAction = 
  ...
  | { type: 'SET_TRANSLATION_PROGRESS'; payload: { progress: number; stage: string; error?: string } }
  | { type: 'SET_MERGE_PROGRESS'; payload: { progress: number; stage: string; error?: string } };

export function ManagementReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    case 'SET_VIDEO_FILE':
