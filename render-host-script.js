...
// packages/shared/constants/subtitle-styles.ts
...
// packages/shared/helpers/subtitle-style-util.ts
...
// packages/renderer/render-host-script.tsx
  ...
  window.updateSubtitle = (text, opts = {}) => {
    ...
    if (stylePreset === "LineBox") {
      ...
      const html = text.split("\n").map(
        (line2) => `<span style="background-color:${bg};padding:1px 6px;display:inline;line-height:1.55;white-space:pre-wrap;box-decoration-break:clone;-webkit-box-decoration-break:clone;">${esc(line2)}</span>`
      ).join("<br/>");
      el.innerHTML = html;
    } else {
      el.textContent = text;
