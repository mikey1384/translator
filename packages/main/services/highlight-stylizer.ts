    ...
    .map(seg => {
      ...
      if (seg.words && seg.words.length > 0) {
        const words = seg.words
          ...
          .map(w => {
            const absStart = Number(w.start);
            const absEnd = Number(w.end);
            const relStart = Math.max(0, absStart - start);
            const relEnd = Math.max(0, absEnd - start);
            return {
        ...
        dialogueText = buildKaraokeLine(words, duration);
      } else {
        dialogueText = escapeAssText(seg.text || '');
      }

      return `Dialogue: 0,${toAssTimestamp(start)},${toAssTimestamp(
        end
