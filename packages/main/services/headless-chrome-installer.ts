 ...
 */
async function isHeadlessChromeBinaryValid(executablePath: string): Promise<boolean> {
  try {
 ...
 */
async function findExecutableInNestedStructure(chromeDir: string): Promise<string | null> {
  try {
    ...
    const versionDir = versionDirs.find(dir => dir.startsWith(versionDirPattern));
    if (!versionDir) return null;
    
    const platformDir = path.join(chromeDir, versionDir);
    const platformDirs = await fs.readdir(platformDir);
    const binaryDir = platformDirs.find(dir => dir.startsWith(binaryDirPattern));
    ...
    log.warn(`[HeadlessChrome] Error finding executable in nested structure: ${error}`);
    return null;
  }
}
    ...
    try {
      ...
      if (!isNaN(pid)) {
        ...
        try {
          ...
          log.info(`[HeadlessChrome] Installation already in progress (PID: ${pid})`);
          return false;
        } catch (error: any) {
          if (error.code === 'ESRCH') {
            // Process not found, remove stale lock
            log.info(`[HeadlessChrome] Removing stale lock file (PID ${pid} not found)`);
...
async function releaseInstallLock(lockFile: string): Promise<void> {
  try {
    ...
    if (error.code !== 'ENOENT') {
      log.warn(`[HeadlessChrome] Failed to release installation lock: ${error}`);
    }
  }
}
...
async function downloadHeadlessChrome(headlessDir: string): Promise<void> {
  log.info(`[HeadlessChrome] Downloading chrome-headless-shell@stable to ${headlessDir}`);
...
export async function ensureHeadlessChrome(): Promise<string> {
  ...
  if (!(await acquireInstallLock(lockFile))) {
    ...
    log.info(`[HeadlessChrome] Waiting for concurrent installation to complete...`);
    ...
    throw new Error('Headless Chrome installation timeout - concurrent installation did not complete');
  }
  ...
  const bundledPath = (() => {
    const headlessDir = path.join(
      ...
      process.arch === 'arm64' ? 'headless-arm64' : 'headless-x64'
       ...
       log.warn(`[HeadlessChrome] Error checking bundled nested structure: ${error}`);
     }
