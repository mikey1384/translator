...
function add(level: LogLevel, kind: LogKind, message: string, meta?: Record<string, any>) {
  try {
...
export const logTask = (phase: 'start' | 'complete' | 'cancel', task: 'transcription' | 'translation' | 'merge', meta?: Record<string, any>) =>
  add(phase === 'start' ? 'info' : phase === 'cancel' ? 'warn' : 'info', 'task', `${task}:${phase}`, meta);

export const logProgress = (action: 'open' | 'close', which: 'transcription' | 'translation' | 'merge') =>
  add('info', 'progress', `${which}:${action}`);

export const logError = (where: string, error: any, meta?: Record<string, any>) => {
...
export const logPhase = (
  ...
  const msg = `${task}:phase:${stage}` + (typeof percent === 'number' ? ` (${Math.round(percent)}%)` : '');
  add('info', 'task', msg, meta);
};

export const logSystem = (info: Record<string, any>) => add('info', 'system', 'device_info', info);
